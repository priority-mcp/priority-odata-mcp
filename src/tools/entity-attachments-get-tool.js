...
export function registerEntityAttachmentsGetTool(registry, client) {
    registry.registerTool({
        name: 'entity_attachments_get',
        description: 'Get attachments for an entity. Retrieves list of files attached to an entity record. Args: { entity: string, key: string }',
        inputSchema: {
            type: 'object',
        ...
        try {
            return await client.getEntityAttachments(
                entityName, 
                String(args.key)
            );
        } catch (error) {
