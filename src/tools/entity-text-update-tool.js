...
export function registerEntityTextUpdateTool(registry, client) {
    registry.registerTool({
        name: 'entity_text_update',
        description: 'Update text for an entity. Modifies existing text content for an entity record. Args: { entity: string, key: string, textData: object }',
        inputSchema: {
            type: 'object',
            ...
            properties: {
                ...
                textData: { 
                    type: 'object', 
                    description: 'Text data to update. Typically contains fields like TEXT, TEXT2, etc. depending on the entity type.' 
                }
            }
        }
    }, async (args) => {
        ...
        try {
            return await client.updateEntityText(
                entityName, 
                String(args.key),
                args.textData
            );
